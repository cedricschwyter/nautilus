...
namespace nautilus {
    ...
    void NautilusCameraFPS::key(
        ...
        int             _action, 
        int             _mods) {
        if(_key == GLFW_KEY_W && _action == GLFW_PRESS) move(NAUTILUS_CAMERA_MOVEMENT_DIRECTION_FORWARD);
        if(_key == GLFW_KEY_A && _action == GLFW_PRESS) move(NAUTILUS_CAMERA_MOVEMENT_DIRECTION_LEFT);
        if(_key == GLFW_KEY_S && _action == GLFW_PRESS) move(NAUTILUS_CAMERA_MOVEMENT_DIRECTION_BACKWARD);
        if(_key == GLFW_KEY_D && _action == GLFW_PRESS) move(NAUTILUS_CAMERA_MOVEMENT_DIRECTION_RIGHT);
    }
    ...
    void NautilusCameraFPS::cursor(GLFWwindow* _window, double _x, double _y) {
        ...
        if(m_inputEnabled) {
            ...
            m_angles.m_pitch = std::clamp(m_angles.m_pitch, defaults::CAMERA_MIN_PITCH, defaults::CAMERA_MAX_PITCH);
            update();
        }
    }
